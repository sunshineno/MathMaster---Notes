import { BookOpen, Download, ExternalLink, HelpCircle, Search, ShieldCheck, Upload, X } from "lucide-react";
import { useState, type ChangeEvent } from "react";
import { APP_VERSION, BUILD_COMMIT, BUILD_DATE_LABEL, VERSION_LABEL } from "../version";
export type SaveStatus = "saved" | "saving" | "error";
interface AppTopBarProps { search:string; onSearchChange:(value:string)=>void; saveStatus:SaveStatus; saveError:string; persistentStorage:boolean; onBackup:()=>void; onRestore:(event:ChangeEvent<HTMLInputElement>)=>void; canInstall:boolean; onInstall:()=>void; }
export default function AppTopBar({search,onSearchChange,saveStatus,saveError,persistentStorage,onBackup,onRestore,canInstall,onInstall}:AppTopBarProps){
